...
A primeira consulta mostra os valores de `DNO` e `SALARY` da tabela `EMPLOYEE`.

```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE
```

A segunda consulta remove os valores duplicados, mas não há valores duplicados para serem removidos, assim ela resulta no mesmo que a primeira consulta.

```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
```

A terceira consulta adiciona uma ordenação primariamente por `DNO` e segundamente por `SALARY`.
