...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES / 3.0 AS ADJUSTED_ACRES FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE+50)/2.0 AS ADJUSTED_FEE FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES*0.02) AS AVG_NEW_FEE FROM PRESERVE
WHERE STATE = 'AZ'
```
